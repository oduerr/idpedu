...
test_that("HTML renders callouts and copies artifacts", {
  skip_on_cran()
  skip_if_no_quarto()
  ...
  tasks <- c(test_path("fixtures/exercise_callouts.qmd"))
  title <- "Callouts Demo"
  fname <- "wk_callouts"
  header_file <- "da.qmd"
  ...
  # Sanity: presence of callout markup and solution class in lsg
  lsg <- readLines(html_lsg, warn = FALSE)
  expect_true(any(grepl('class=\"callout', lsg)))
  expect_true(any(grepl('class=\"[^"]*solution[^"]*\"', lsg)))
})

test_that("PDF renders callouts (via Pandoc) and copies artifacts", {
  skip_on_cran()
  skip_if_no_quarto()
  skip_if_no_pdflatex()
  testthat::skip_if_not_installed("pdftools")
  ...
  tasks <- c(test_path("fixtures/exercise_callouts.qmd"))
  title <- "Callouts Demo PDF"
  fname <- "wk_callouts_pdf"
  header_file <- "da.qmd"
